...
A critical assumption underlying these EPS predictions is that a halo is the dominant peak in its large-scale environment. This is a well-founded assumption at the high-mass end, and we see that the predictions are in good agreement with simulations in the $\mvir>\mcoll$ regime \citep{dalal_etal08}. The situation is quite different when $\mvir\lesssim\mcoll.$ We have confirmed previous results \citep[e.g.,][]{wechsler06} that assembly bias changes sign and strengthens for lower-mass halos. Meanwhile, the EPS model described above makes the same prediction for assembly bias regardless of halo mass. Thus EPS fails in precisely the regime where we expect: for lower-mass halos that are strongly influenced by the nonlinear tidal field in which they evolve \citep{hahn_etal07b,hahn_etal09,hearin_etal15}, there is a catastrophic breakdown of the assumption that the halo dominates its environment.

% \citet{wang_mo_jing08}
