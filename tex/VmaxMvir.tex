...
\subsection{Assembly Bias of High-Mass Halos}
For halos $M_{{\rm vir}}\gtrsim\mcoll\approx10^{12.5}\msun,$ we have shown that the linear bias of low-$V_{{\rm max}}$
...
\foonote{See also Chapter IX, Section D of \citet{zentner07} for a similar explanation deriving from an alternative formulation of EPS.} 
Consider two halos with the same present-day mass, but with different concentration. Both halos originate from a 
statistical fluctuation of the same peak height, but with different peak curvature: the high-concentration (high-$\vmax$) halo 
...
\subsection{Assembly Bias of Low-Mass Halos}
