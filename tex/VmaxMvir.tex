...
In this section, we describe our methodology for studying how halo clustering depends on $\mvir$ and $\vmax.$  We first start in \S\ref{subsec:vmaxdef} by defining the analytic expression for $\vmax$ that we use to categorize halos as having above- or below-average circular velocities for their mass. In \S\ref{subsec:samples} we describe the halo samples we use throughout the paper.  

\subsection{The Maximum Circular Velocity}
\label{subsec:vmaxdef}
