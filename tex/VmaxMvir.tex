...
{Tomomi Sunayama$^1$, Andrew P. Hearin$^2$, Nikhil Padmanabhan$^1$, Alexie Leauthaud$^3$\\
...
the halo catalogues. {\bf APH: The following sentence must be corrected: The halo mass and velocity functions start to
show incompleteness at $10^{11}h^{-1}M_{\odot}$ ($\sim740$ particles)
for the Bolshoi simulation, and $10^{12}h^{-1}M_{\odot}$ ($\sim100$
particles) for the MultiDark simulation. }
%TS: The sentence below is not correct; I use halos greater than 10^12 solar masses for the MultiDark. I don't use the conservative cut.
...
%TS: Is the above statement really correct? Original peak-background split and PS formalism do not predict assembly bias...It's predicted by the paper by Kaiser...
% APH: Yes, this sentence is correct as written. You are right that neither p-b split nor PS predict assembly bias. However, the sentence refers to the basic shape of *each curve*, not the *relative* shape of the two curves. Feel free to alter this language to something more intuitive if you find it confusing. 
...
the green lines correspond to low-$V_{{\rm max}}$ halos. The solid
lines are the case of including the ejected halos and the dashed lines
are the case without them. Note that the MultiDark simulation does not 
...
into a series of bins with constant number density (=$1.6\times10^{-3}(h^{-1}{\rm Mpc})^{-3}$) {\bf APH: What does this number mean?},
...
This scale-dependence has a simple interpretation in terms of subhalo back-splashing. First, by definition, ejected halos are
